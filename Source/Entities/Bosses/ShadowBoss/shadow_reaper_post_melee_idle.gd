...
func update(delta: float) -> void:
	...
	if _mode == MoveMode.ORBIT:
		if character.has_method("reaper_orbit_target_post_melee"):
			character.reaper_orbit_target_post_melee(delta)
	else:
		if character.has_method("reaper_move_toward_target_post_melee"):
			character.reaper_move_toward_target_post_melee(delta)
...
func check_transition(_delta: float) -> TransitionData:
	...
	if works_longer_than(_get_duration()):
...
func _get_duration() -> float:
	if character and character.has_method("get_reaper_post_melee_idle_duration"):
		return character.get_reaper_post_melee_idle_duration()
	return 1.0
