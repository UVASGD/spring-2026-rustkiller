...
func teleport_next_to_target() -> void:
	...
	boss.global_position = boss.player.global_position + Vector2(side * boss.slash_teleport_offset, 0.0)
	boss.stop_motion()
	boss.face_target()
...
func teleport_close_to_target_for_phase_transition() -> void:
	...
	boss.global_position = boss.player.global_position + Vector2(side * boss.phase_transition_teleport_offset, 0.0)
	boss.stop_motion()
	boss.face_target()
