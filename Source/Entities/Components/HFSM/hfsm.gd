# TAKEN FROM: https://www.youtube.com/watch?v=2UjeZFzzfB0,
# https://github.com/Gab-ani/Godot-3D-Enemies/tree/Ep3-HFSM-and-Boss
extends Node
class_name HFSM
...
## Transitions to a state on the same layer.
...
## Transitions to a state in a lower layer.
...
# The top level state machine needs the method called from somewhere, usually the boss node (highest)
...
#region (IGNORE!) IrrelevantWeaponData
...
func projected_direction_to_player() -> Vector2:
	return character.global_position.direction_to(get_projected_player_pos())

func get_projected_player_pos() -> Vector2:
